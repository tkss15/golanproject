...
export async function getProjects(limit: number, offset: number, departmentId?: number, funderId?: number, userId?: number) {
    // Handle filter combinations
    if (departmentId && funderId) {
        return getProjectsByDepartmentAndFunder(limit, offset, departmentId, funderId, userId);
    } 
    
    if (funderId) {
        return getProjectsByFunder(limit, offset, funderId, userId);
    } 
    
    if (departmentId) {
        return getProjectsByDepartmentC(limit, offset, departmentId, userId);
    }
    
    // Get all projects (will be filtered for user access if userId is provided)
    return getAllProjects(limit, offset, userId);
}

// Function for getting projects by both department and funder
async function getProjectsByDepartmentAndFunder(limit: number, offset: number, departmentId: number, funderId: number, userId?: number) {
...
async function getProjectsByDepartmentC(limit: number, offset: number, departmentId: number, userId?: number) {
...
async function getProjectsByFunder(limit: number, offset: number, funderId: number, userId?: number) {
...
async function getAllProjects(limit: number, offset: number, userId?: number) {
...
export async function getMyProjects(limit: number, offset: number, userId: string, departmentId?: string) {
    if (!userId) {
    ...
    return getProjects(limit, offset, departmentIdNum, undefined, userIdNum);
}

// Public API: Count my projects (where I'm owner or editor)
export async function getMyProjectsCount(userId: string, departmentId?: string) {
    if (!userId) {
        throw new Error('User ID is required');
    }

    const userIdNum = parseInt(userId);
    const userFilter = createUserAccessFilter(userIdNum);
    
    // Apply departmentId filter if provided
    const filters = departmentId 
        ? and(userFilter, eq(projects.department_id, parseInt(departmentId)))
        : userFilter;
